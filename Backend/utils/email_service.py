...
def enviar_correo_reserva(nombre, correo_destino, servicio_nombre, fecha, hora):
    """
    Envía un correo cuando el paciente solicita una reserva.
    """
    ...
    def _enviar():
        ...
        <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 600px; margin: 0 auto; background-color: #f9fdfa; border: 1px solid #e2f0e8; border-radius: 10px; overflow: hidden;">
            <div style="background-color: #2b7a5a; color: white; padding: 20px; text-align: center;">
                <h2 style="margin: 0; font-size: 24px;">¡Reserva Recibida! 🌿</h2>
...
def enviar_correo_cita_confirmada(nombre, correo_destino, servicio_nombre, fecha, hora):
    """
    Envía un correo de confirmación cuando el admin aprueba la cita.
    """
    if not correo_destino:
        return
    ...
    def _enviar():
        ...
        <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 600px; margin: 0 auto; background-color: #f9fdfa; border: 1px solid #e2f0e8; border-radius: 10px; overflow: hidden;">
            ...
            <div style="padding: 30px; color: #333;">
                <p style="font-size: 16px;">Hola <strong>{nombre}</strong>,</p>
                <p style="font-size: 16px; line-height: 1.5;">Nos alegra informarte que tu cita ha sido <strong>confirmada</strong> oficialmente. Aquí están los detalles:</p>
                ...
                <p style="font-size: 15px; color: #666; line-height: 1.5;">Si necesitas cancelar o reprogramar, por favor contáctanos con anticipación a través de nuestro Bot en Telegram.</p>
                <p style="font-size: 16px; margin-top: 30px;">¡Te esperamos!</p>
            </div>
        </div>
        """
        
        payload = {
            "sender": {"name": "Fisioterapeuta Li", "email": remitente},
            "to": [{"email": correo_destino, "name": nombre}],
            "subject": asunto,
            "htmlContent": html
        }
